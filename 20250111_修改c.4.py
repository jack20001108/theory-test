...
def evaluator(demand, order, leadtime, begin_inventory, bom):  # 輸入一組需求與訂購資料 輸出缺貨矩陣(缺貨時間,缺貨數量) 
    ...
    real_stock = np.where(inventory > 0, inventory, 0)
    #紀錄缺貨數量
...
def fitness(order, num_products=3, num_periods=5):
    ...
    for demand in demand_samples:
        material_demand, inventory, shortage, material_unclaim, product_shortage,real_stock = evaluator(demand, order, leadtime, begin_inventory, bom)
...
def fitness_function(individual,gen,num_generations):
    ...
    #print(final_fill_rate)
    individual.fill_rate = final_fill_rate
...
num_periods = 5  
leadtime = 1
begin_inventory = np.array([100, 200, 100, 100])
bom = np.array([
    [1, 0, 1],
    [2, 1, 0],
    [0, 1, 1],
...
toolbox.register("attr_int", np.random.randint, 0,500)  #每個基因的值（訂單 0 ~ 200）
...
# 計算最佳方案對應的平均需求
_, _, _, avg_demand,real_stock = fitness(best_order, num_products, num_periods)
# 獲取最佳方案的滿足率
best_fill_rate = best_solution.fill_rate

# 輸出結果
print(f"最佳訂單方案：\n{best_order}")
print(f"最佳解的目標值（庫存成本 + 懲罰）：{best_fitness[0]}")
print(f"平均需求：\n{avg_demand}")
print(f"滿足率（fill rate）：\n{best_fill_rate}")
...
for product in range(num_products):
    plt.plot(np.arange(num_periods), best_fill_rate[product], label=f"Product {product + 1}")
plt.xlabel("Time Period")
plt.ylabel("Fill Rate")
plt.title("Fill Rate Curve for Best Solution")
plt.legend()
plt.grid(True)
plt.show()
