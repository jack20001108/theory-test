...
# 修正版本：展開 BOM 需求
def expand_bom_demand(demand_samples, bom_matrix, num_period):
    # 初始化一個字典來存放每個零件的需求
    component_demand_breakdown = {}
    ...
    for component_id, component_name in enumerate(components):
        ...
        for product_id in range(len(finish_product)):
            # 計算單個產品對零件的需求貢獻
            product_demand_contribution = (
                demand_samples[product_id][:, :num_period] * bom_matrix[product_id, component_id]
            )
            component_breakdown.append(product_demand_contribution)

        # 將需求分解矩陣合併
        component_breakdown_matrix = np.array(component_breakdown)  # Shape: (3, num_sample, num_period)

        # 建立 DataFrame：列為產品，欄為期數
        breakdown_df = pd.DataFrame(
            component_breakdown_matrix.reshape(len(finish_product), -1),  # 將每個產品需求展開
            index=finish_product,
            columns=[f"Period {i+1}" for i in range(num_period)],
        )
        component_demand_breakdown[component_name] = breakdown_df

    return component_demand_breakdown
...
def cal_cost(demand,order,leadtime,hold_cost,order_cost):
    ...
    for period in range(len(demand)):
        ...
        #分配零件
        for component_to_product in enumerate(finish_product):
            unclaim_inv_level= inventory 
            print(f"unclaimed inventory level of product{component_to_product}:{unclaim_inv_level}")
...
# 呼叫修正的函數
component_demand_breakdown = expand_bom_demand(demand_samples, bom_matrix, num_period)

# 將結果輸出
for component_name, breakdown_df in component_demand_breakdown.items():
    print(f"Breakdown of demand for {component_name}:")
    print(breakdown_df)
    print("\n")
