...
def visualize_sampling_convergence(order, sampling_function, title):
    sample_sizes = []
    means = []
    stds = []
    ...
    while not converged:
        ...
        # Check convergence
        if abs(sample_mean - lam) < epsilon and abs(sample_std - sigma_threshold) < epsilon:
            converged = True
        else:
            sample_size += 50
            if sample_size > max_sample_size:
    ...
    plt.plot(sample_sizes, means, label="Sample Mean", marker="o")
    plt.axhline(y=lam, color="r", linestyle="--", label="Target Mean (λ)")
    plt.xlabel("Sample Size")
    plt.ylabel("Sample Mean")
    plt.title(f"{title}: Sample Mean Convergence")
    plt.legend()
    plt.grid()
    plt.show()
    ...
    plt.figure(figsize=(10, 6))
    plt.plot(sample_sizes, stds, label="Sample Std", marker="o")
    plt.axhline(y=sigma_threshold, color="g", linestyle="--", label="Target Std (σ)")
    plt.xlabel("Sample Size")
    plt.ylabel("Sample Std")
    plt.title(f"{title}: Sample Std Convergence")
    plt.legend()
    plt.grid()
    plt.show()


# Visualize convergence for both LHS and RS
visualize_sampling_convergence(order, latin_hypercube_sampling, "LHS Sampling")
visualize_sampling_convergence(order, random_sampling, "Random Sampling")
