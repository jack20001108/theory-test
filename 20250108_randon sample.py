...
def simulate_with_dynamic_samples(
    ...
    for iteration in range(max_iterations):
        ...
        for demand in demand_samples:
            ...
            print(f"第 {iteration + 1} 次迭代: 當前樣本的平均填充率 = {np.mean(fill_rate):.4f}")
            print(f"第 {iteration + 1} 次迭代: 累積平均填充率 = {cumulative_mean:.4f}")
...
# 執行模擬
samples, final_shortage_mean, total_samples = simulate_with_dynamic_samples(
...
print(f"最終平均缺貨數量：{final_shortage_mean:.4f}")
