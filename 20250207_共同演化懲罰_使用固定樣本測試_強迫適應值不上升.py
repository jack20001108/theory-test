...
def fitness(order):
    ...
    demand_samples = np.floor(lhs_samples * (max_demand - min_demand + 1) + min_demand).astype(int) # 均勻分配轉整數
...
def genetic_algorithm():
    ...
    penalty_population = toolbox.penalty_population(n=10)  

    hall_of_fame = tools.HallOfFame(1)  # 只保留「最佳」個體
    ...
    best_fitness_history = []  # 記錄最佳適應值
    global_best_individual = None  # 全局最佳個體
    ...
    for gen in range(num_generations): 
        ...
        # **只保留菁英**
        population = combined_selection(offspring, len(offspring), elitism_size=10)

        # **確保最佳個體不會被取代**
        hall_of_fame.update(population)
        best_fitness = hall_of_fame[0].fitness.values[0]

        if global_best_individual is None or best_fitness < global_best_individual.fitness.values[0]:
            global_best_individual = hall_of_fame[0]  # 更新全局最佳個體
        else:
            population[0] = global_best_individual  # 確保全局最佳不會消失
        ...
        if len(best_fitness_history) > 0 and best_fitness > best_fitness_history[-1]:
            print(f"⚠️ Generation {gen}: Fitness increased! Reverting to previous best.")
            best_fitness = best_fitness_history[-1]
            population[0] = global_best_individual  # 強制回溯
        ...
        for ind in offspring_penalty:
            ind.fitness.values = evaluate_penalty(ind,gen, sample_size_history)
        penalty_population = tools.selBest(offspring_penalty, k=len(penalty_population))  
...
def combined_selection(population, k, elitism_size):
    ...
    if global_best not in best_individuals:
        best_individuals[0] = global_best  # 確保全局最優解被保留
...
print("Execution Time of GA: ", end_time_ga - start_time_ga)
