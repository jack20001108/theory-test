...
def evaluator(demand, order, leadtime, begin_inventory, bom):  # 輸入一組需求與訂購資料 輸出缺貨矩陣(缺貨時間,缺貨數量) 
    ...
    shortage[:,1:]=np.where(cummulate_shortage[:,1:]==0,np.zeros_like(cummulate_shortage[:, 1:]),cummulate_shortage[:, 1:] - cummulate_shortage[:, :-1])

    shortage_matrix=np.where(inventory[:, 1:] < 0, 1, 0)
    #print("shortage_matrix", shortage_matrix)
    # 計算反累積需求 (全部倒過來累加)
    ...
    product_shortage_matrix=np.where(product_shortage<0,1,0)
