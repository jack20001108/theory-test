...
def evaluator(demand, order, leadtime, begin_inventory, bom):  # 輸入一組需求與訂購資料 輸出缺貨矩陣(缺貨時間,缺貨數量) 
    ...
    bom_demand_4_5 = bom_demand.sum(axis=1)
    print("bom_demand_4_5", bom_demand_4_5)
   ...
   #紀錄缺貨數量
    ...
    product_shortage_matrix=np.where(product_shortage<0,0,1) #有缺貨記0 沒缺貨記1


    return material_demand,inventory,shortage,material_unclaim,product_shortage,product_shortage_matrix 
...
def fitness(order, num_products=3, num_periods=5):
    ...
    for demand in demand_samples:
        material_demand, inventory, shortage, material_unclaim, product_shortage, product_shortage_matrix = evaluator(demand, order, leadtime, begin_inventory, bom)
        ...
        product_fill_rate_matrix=np.prod(product_shortage_matrix, axis=1) #產品滿足率矩陣
        product_fill_rate=np.sum(product_fill_rate_matrix, axis=1)/num_periods #產品滿足率
        print("Product fill rate matrix:", product_fill_rate_matrix)
        print("Product fill rate:", product_fill_rate)
        cumulative_fill_rate.append(product_fill_rate)
...
def fitness_function(individual,gen,num_generations):
    ...
    if np.any(final_fill_rate < target_fill_rate):
        ...
        beta = 0.9  # 折扣率
        # 計算每期的權重 w_t
        period_weights = np.array([beta**t for t in range(num_periods)])
